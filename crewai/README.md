...
The point isn't this specific scenario — it's the **tool-wrapping pattern**: a CrewAI `tool` whose `_run` method makes an x402-paid HTTP call, returning structured output the LLM can reason over.
...
- Deterministic output the LLM can cite by name ("IRR was 18.2% per Hermes DealAnalyzer") instead of asking the LLM to guess the math.

For other Hermes endpoints — Black-Scholes Greeks, bond analytics, AML screening — copy `HermesDealAnalyzerTool` and change the endpoint path + input schema.
